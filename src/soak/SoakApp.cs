...
static class SoakApp
{
    static int Main(string[] args)
    {
        try
        {
            ...
            {
                string hlp = $"Soaks up all inputs before returning them back." +
                    $"\nVersion {Version}" +
                    $"\nUsage:\t{Usage}" +
    ...
    static string Version
    {
        get
        {
            Version? version = typeof(SoakApp).Assembly.GetName().Version;
    ...
    static void Run() => IOManager.WriteLines(IOManager.ReadLines().ToList());
}
