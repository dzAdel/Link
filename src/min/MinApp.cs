...
sealed class MinApp
{
    ...
    static int Main(string[] args)
    {
        try
        {
            ...
            {
                string hlp = "Returns the minimum entry in the input sequence." +
                    $"\nVersion {Version}" +
                    $"\nUsage:\t{Usage}" +
                    ...
                    "\ntsuch as capturing group that is identified by a number or a name." +
    ...
    static string Version
    {
        get
        {
            Version? version = typeof(MinApp).Assembly.GetName().Version;
    ...
    void Run()
    {
        ...
        string? min = IOManager.ReadLines().Min(comparer);

        if (min != null)
            IOManager.WriteLine(min);
    }
    ...
    void Run(string rxPattern, string rxKey)
    {
        ...
        string min = enumerator.Current;
        string minKey = match.Result(rxKey);
