...
sealed class PadStartApp
{
    public PadStartApp(int len)
    {
        require(len >= 0);
    ...
    static int Main(string[] args)
    {
        try
        {
            ...
            {
                string hlp = "Returns, for each line in the input sequence, a new string of a specified length in which " +
                    "the beginning of " +
                    "the input line is padded with spaces or with a specified character. This command pads the " +
                    "beginning of the returned string. This means that, when used with right-to-left languages, " +
                    "it pads the right portion of the string." +
                    $"\nVersion {Version}" +
                    $"\nUsage:\t{Usage}" +
    ...
    static string Version
    {
        get
        {
            Version? version = typeof(PadStartApp).Assembly.GetName().Version;
    ...
    void Run()
    {
        foreach (string s in IOManager.ReadLines())
            IOManager.WriteLine(s.PadLeft(Length, PaddingChar));
    }
}
