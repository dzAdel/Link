...
sealed class FilesExtApp
{
    ...
    static int Main(string[] args)
    {
        try
        {
            ...
            {
                string hlp = "Returns the extension (including the period \".\") of every file path in the input sequence. " +
                    ...
                    "\n\t‘file’ is a transform pattern to apply to each input line to extract the target file path." +
    ...
    static string Usage => $"{AppName} [--rx ptrn file [--cs]][--pi [sep]][--h]";
    ...
    static string Version
    {
        get
        {
            Version? version = typeof(FilesExtApp).Assembly.GetName().Version;
        ...
        foreach (string path in IOManager.ReadLines())
            try
            {
                ...
                if (!string.IsNullOrEmpty(ext))
                    IOManager.WriteLine(PrependInput ? $"{path}{m_sep}{ext}" : ext);
            }
            catch (Exception ex)
            {
                IOManager.LogError($"{AppName}: {ex.Message}");
    ...
    void Run(string rxPattern, string rxPath)
    {
        ...
        foreach (string str in IOManager.ReadLines())
        {
            try
            {
                ...
                if (m.Success)
                {
                    string? ext = Path.GetExtension(m.Result(rxPath));

                    if (!string.IsNullOrEmpty(ext))
                        IOManager.WriteLine(PrependInput ? $"{str}{m_sep}{ext}" : ext);
                }
            }
            catch (Exception ex)
            {
